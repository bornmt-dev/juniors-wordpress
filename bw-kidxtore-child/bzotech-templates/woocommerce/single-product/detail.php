    ...
    <div class="bzotech-row">
            ...
            <div class="product-detail-gallery-js product-detail-gallery <?php if($tab_style == 'tab-product-accordion2') echo 'tab-accordion-gallery-sticky'; ?> ">
                <div class="wrap-detail-gallery images <?php echo esc_attr($zoom_style)?>">
                    <div class="mid woocommerce-product-gallery__image image-lightbox" data-gallery="<?php echo esc_attr($gallerys)?>">
                        ...
                        if ( !$html ) {
                            ?>
                            <img src="/wp-content/uploads/2025/07/placeholder-1.webp" alt="Image" decoding="async">
                            <?php
                        }
                       ?>
                    ...
                    if ( $attachment_ids && has_post_thumbnail() && count($attachment_ids) > 1) {
                        ...
                        <div class="gallery-control">
                            <div class="gallery-slider carousel" data-visible="4">
                                ...
                                ?>
                                <?php 
                                $bzotech_video_product = get_post_meta(get_the_ID(),'bzotech_video_product',true);
                                if(!empty($bzotech_video_product)) { ?>
                                        <a class="slick-slide video-product-detail-poup" href="<?php echo esc_url($bzotech_video_product);?>"><div><p><i class="las la-play title40"></i><span class="title14"><?php echo esc_html__('Watch video','bw-kidxtore');?></span></p>
                                            <?php echo wp_get_attachment_image(get_post_thumbnail_id(),$size_gallery); ?>
                                            </div>
                                        </a>
                                <?php } ?>
                            </div>
        ...
        <div class="<?php echo esc_attr($info_class)?>">
            <div class="summary entry-summary product-detail-info <?php if($tab_style == 'tab-product-accordion2') echo 'tab-accordion-info-sticky'; ?> ">
                <h1 class="product-title-single font-title title30 font-medium color-title"><?php the_title()?></h1>
                <?php
                /**
                ...
                <?php 
                $stocks_per_location = getStockPerLocationByID($product->get_id());
                ?>
                <div id="block-stocks-per-locations">
                    <?php
                        foreach( $stocks_per_location as $data ) {
                            if ( $data["is_main"] == true) {
                                ?>
                                <div> 
                                   <p class="bsprl-label"> Online </p> 
                                   <p class="bsprl-value <?php echo $data['class']; ?>"> <?php echo $data['text']; ?> </p> 
                               </div>
                               <?php
                            } 
                        }
                    ...
                    <div> 
                        <p> Stock available in store at: </p> 
                    </div>
                    ...
                    <?php
                        foreach( $stocks_per_location as $data ) {
                            ?>
                             <div> 
                                <p class="bsprl-label"> <?php echo $data['location']; ?> </p> 
                                <p class="bsprl-value <?php echo $data['class']; ?>"> <?php echo $data['text']; ?> </p> 
                            </div>
                            <?php
                        }
