...
if (!empty($brands) && !is_wp_error($brands)) {
    foreach ($brands as $brand) {
        $brand_class .= ' brand-' . sanitize_html_class($brand->slug);
    }
}
$product_id_class = ' product-id-' . $product_id;

// Check if product has a featured image
$no_image_class = has_post_thumbnail() ? '' : ' no-image';

// Build the class value for the product wrapper
$class_value = trim(str_replace('class="', '', rtrim($item_wrap, '"')));
$class_value .= $brand_class . $product_id_class . $no_image_class;  // Add the no-image class if no thumbnail
$item_wrap = 'class="' . trim($class_value) . '"';
// end of custom
...
<?php if($view !== 'slider-masory') echo '<div '.$item_wrap.'>';?>
	<?php if($view !== 'slider-masory') echo '<div '.$item_inner.'>';?>
		...
		<?php if($item_thumbnail == 'yes' && !has_post_thumbnail()):?>
			<div class="product-thumb attribute-close product">
				<!-- bzotech_woocommerce_thumbnail_loop have $size and $animation -->
				<?php if($item_label == 'yes') bzotech_product_label()?>
				<a href="<?php the_permalink(); ?>" class="product-thumb-link rotate-thumb">
                    <img width="400" height="400" src="/wp-content/uploads/2025/07/placeholder-1.webp" class="attachment-400x400 size-400x400 wp-post-image" alt="" decoding="async">
                    <img width="400" height="400" src="/wp-content/uploads/2025/07/placeholder-1.webp" class="image-hover wp-post-image" alt="" decoding="async">
                </a>							
			</div>
		<?php endif?>
