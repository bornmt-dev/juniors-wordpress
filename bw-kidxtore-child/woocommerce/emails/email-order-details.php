...
<div style="margin-bottom: <?php echo $email_improvements_enabled ? '24px' : '40px'; ?>;">
	<table class="td font-family <?php echo esc_attr( $order_table_class ); ?>" cellspacing="0" cellpadding="6" style="width: 100%;" border="1">
		...
		<tfoot>
				...
				foreach ( $item_totals as $total ) {
					...
					<tr class="order-totals order-totals-<?php echo esc_attr( $total['type'] ?? 'unknown' ); ?><?php echo esc_attr( $last_class ); ?>">
						...
						<th class="td text-align-left" scope="row" colspan="2" style="<?php echo ( 1 === $i ) ? 'border-top-width: 4px;' : ''; ?>">
							...
							<?php
							if ( wp_kses_post( $total['label'] ) == "Shipping:" && wp_kses_post( $total['value'] ) == "Online" ) {
								echo "Online: ";
							}
							else if ( wp_kses_post( $total['label'] ) == "Shipping:" && wp_kses_post( $total['value'] ) == "Pick up In Store" ) {
								echo "Pick up In Store: ";
							}
							else {
						...
						<td class="td text-align-<?php echo esc_attr( $order_total_text_align ); ?>" style="<?php echo ( 1 === $i ) ? 'border-top-width: 4px;' : ''; ?>">
							...
							if ( wp_kses_post( $total['label'] ) == "Shipping:" ) {
								echo bornGetLocationNameByID( $order->get_meta( '_custom_location_id' ) );
			...
			<?php
			// Get the gift receipt value from order meta
			$gift_receipt = $order->get_meta( '_gift_receipt' );
			if ( ! empty( $gift_receipt ) ) : ?>
				<p style="margin-top: 20px;">
					<strong>Gift Receipt:</strong> <?php echo esc_html( ucfirst( strtolower( $gift_receipt ) ) ); ?>
				</p>
			<?php endif; ?>
