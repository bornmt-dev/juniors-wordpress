 ...
 * Time: 10:20 AM
 */
if(!class_exists('Bzotech_Category_Filter') && class_exists("woocommerce"))
{
    class Bzotech_Category_Filter extends WP_Widget {
        ...
        {
            if(function_exists('bzotech_reg_widget')) bzotech_reg_widget( 'Bzotech_Category_Filter' );
        }

        function __construct() {
            // Instantiate the parent object
            parent::__construct( false, esc_html__('Custom Brands Filter','bw-kidxtore'),
        ...
        //             echo        '<ul>';                
        //                 $cat_current = '';
        //                 if(isset($_GET['product_brand'])) $cat_current = sanitize_text_field($_GET['product_brand']);
        //                 if($cat_current != '') $cat_current = explode(',', $cat_current);
        //                 else $cat_current = array();
        //                 foreach ($category as $cat_slug) {
        //                     $cat = get_term_by('slug',$cat_slug,'product_brand');
        //                     if(is_object($cat)){
        //                         if(in_array($cat->slug, $cat_current)) $active = 'active';
        //                         else $active = '';
        //                         echo        '<li><a data-cat='.esc_attr($cat->slug).' href="'.esc_url(bzotech_get_filter_url('product_brand',$cat->slug)).'" class="load-shop-ajax '.$active.'"> '.$cat->name.'</a><span class="smoke">('.$cat->count.')</span></li>';
        //                     }
        //                 }
        //             echo      '</ul>';
        //         }      
        ...
        function widget( $args, $instance ) {
            echo apply_filters( 'bzotech_output_content', $args['before_widget'] );
                ...
                // Fetch all parent brands (no sub-brands)
                $brands = get_terms( array(
                    'taxonomy'   => 'product_brand',
                    'hide_empty' => false, // change to true if you only want brands that have products
                    'parent'     => 0,     // ensures only top-level / parent brands
                ) );
                ...
                if ( isset( $_GET['product_brand'] ) && ! empty( $_GET['product_brand'] ) ) {
                    $active_brands = array_map( 'sanitize_title', explode( ',', $_GET['product_brand'] ) );
                }
                ...
                if ( ! empty( $brands ) && ! is_wp_error( $brands ) ) {
                    foreach ( $brands as $brand ) {
                        ...
                        $active = '';
                        if ( in_array( $brand->slug, $active_brands, true ) ) {
                            $active = 'active';
                        }
        ...
        function form( $instance ) {
            ...
            <p>
                <label><?php esc_html_e( 'Categories:' ,'bw-kidxtore'); ?></label></br>
                <?php 
