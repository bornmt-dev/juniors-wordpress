...
if(!class_exists('Bzotech_Category_Fillter') && class_exists("woocommerce"))
{
    class Bzotech_Category_Fillter extends WP_Widget {
        ...
        function __construct() {
            ...
            parent::__construct( false, esc_html__('BZOTECH Categories Filter','bw-kidxtore'),
                    ...
                    echo        '<ul>';                
                        ...
                        foreach ($category as $cat_slug) {
                            ...
                            if(is_object($cat)){
                                ...
                                //     echo '<li><a data-cat="' . esc_attr( $cat->slug ) . '" href="' . esc_url( bzotech_get_filter_url( 'product_cat', $cat->slug ) ) . '" class="load-shop-ajax ' . $active . '"> ' . esc_html( $cat->name ) . '</a><span class="smoke">(' . intval( $cat->count ) . ')</span></li>';
                                // }
                                // Count in-stock products manually
                                ...
                                if ( $instock_count > 0 ) {
                                    echo '<li><h3><a data-cat="' . esc_attr( $cat->slug ) . '" href="' . esc_url( bzotech_get_filter_url( 'product_cat', $cat->slug ) ) . '" class="load-shop-ajax ' . $active . '"> ' . esc_html( $cat->name ) . '</a></h3><span class="smoke">(' . $instock_count . ')</span></li>';
                                }
                            }
                        }
                    echo      '</ul>';
