			...
			if($link['url']) echo '<a class = "link-bg-banner" href ="'.$link['url'].'" ></a>';
				...
				if ( $list_button ) {
					...
					foreach (  $list_button as $item ) {
						...
						$wdata->add_render_attribute( 'button-inner', 'class', 'button-inner elbzotech-bt-'.$item['style'].' '.$item['add_class_css'].' '.$class_item_custom.' '.bzotech_implode($item['css_by_theme_button']).' elementor-repeater-item-'.$item['_id'] );
