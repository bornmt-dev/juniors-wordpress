...
<?php echo '<div '.$wdata->get_render_attribute_string( 'elbzotech-wrapper' ).'>';?>
    ...
    <?php echo '<div '.$wdata->get_render_attribute_string( 'elbzotech-wrapper-slider' ).'>';?>
        ...
        <div class="info"><h2 class="info-title color-title font-medium title18"><a '.$link_html.'>'.$title_html.'</a></h2><div class="desc">'.$desc_html.'</div></div>';
                    }  
                            
                echo '</div></div>';

                $wdata->remove_render_attribute( 'data_link'.$key, 'target', "_blank" );
                $wdata->remove_render_attribute( 'data_link'.$key, 'rel', "nofollow");
                $wdata->remove_render_attribute( 'data_link'.$key, 'href', $item['link']['url']);
