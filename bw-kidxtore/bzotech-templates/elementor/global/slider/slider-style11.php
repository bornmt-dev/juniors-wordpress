...
<?php echo '<div '.$wdata->get_render_attribute_string( 'elbzotech-wrapper' ).'>';?>
    ...
    <?php echo '<div '.$wdata->get_render_attribute_string( 'elbzotech-wrapper-slider' ).'>';?>
        <?php echo '<div '.$wdata->get_render_attribute_string( 'elbzotech-inner' ).'>';?>
            <?php 
            foreach (  $list_categories6 as $key => $item ) {
                $dem++;
                ...
                echo '<div '.$wdata->get_render_attribute_string( 'elbzotech-item-slider-'.$style.'-'.$key ).'><div class="item-slider-global-'.$style.' item-slider-global-'.$dem.'">';

                    $link_html = $title_html =$desc_html = $image_html =$count_html =$button_html ='';
                    if(!empty($item['category'])){
                        ...
                        if($cat->term_id){
                            ...
                            $count_html = '<span class="cat-count">'.$cat->count.$count_label.'</span>';
                    ...
                    if(!empty($item['template'])) 
                       ...
                       echo '<div class="item-img"><a class="adv-thumb-link-cate adv-thumb-link" '.$link_html.'>'.$image_html.'</a></div>
                            <div class="info"><h2 class="info-title"><a '.$link_html.'>'.$title_html.'</a></h2>';
                    }
                    if(!empty($item['button_name'])){
                        echo '<div class="btn-explore"><a '.$link_html.'><i class="las la-plus-circle"></i>'.$item['button_name'].'</a></div>';
                    }
                echo '</div></div></div>';
