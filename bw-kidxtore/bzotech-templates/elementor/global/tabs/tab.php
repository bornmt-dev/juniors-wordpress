...
foreach ( $tabs as $key => $tab ) : 
	...
	if(!empty($icon_image['url'])){
		...
		if( $icon['library'] == 'svg')
			$header_tab_item_html .= '<img alt="'.esc_attr__('svg','bw-kidxtore').'" src="'.$icon['value']['url'].'">';
		else
	...
	$tab_content_html .= '<div id="'.$_id.''.$id_int.'" class="flex-wrapper flex-box-content-tab-e tab-pane '.$active.'">';
		if(!empty($template)) 
...
<div <?php echo apply_filters('bzotech_output_content',$wdata->get_render_attribute_string( 'wrap')) ?>>
	...
	<?php echo '<div class="tab-content">'.apply_filters('bzotech_output_content',$tab_content_html).'</div>'; ?>
</div>
