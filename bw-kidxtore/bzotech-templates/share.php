...
if((isset($check_share[$post_type]) && $check_share[$post_type] == '1') || $check_share_page == '1'):
	$face_icon =bzotech_get_icon_svg('face');
	$envelope_icon =bzotech_get_icon_svg('envelope');
	$twitter_icon =bzotech_get_icon_svg('twitter');
	$pinterest_icon =bzotech_get_icon_svg('pinterest');
	$list_default = array(
		array(
			'title'  => esc_html__('Total','bw-kidxtore'),
		    'social' => 'total',
		    'number' => '1',
			),
		array(
			'title'  => esc_html__('Facebook','bw-kidxtore'),
		    'social' => 'facebook',
		    'number' => '1',
			),
		array(
			'title'  => esc_html__('Twitter','bw-kidxtore'),
		    'social' => 'twitter',
		    'number' => '1',
			),
		array(
			'title'  => esc_html__('Pinterest','bw-kidxtore'),
		    'social' => 'pinterest"',
		    'number' => '1',
			),
		array(
			'title'  => esc_html__('Linkedin','bw-kidxtore'),
		    'social' => 'Linkedin',
		    'number' => '1',
			),
		array(
			'title'  => esc_html__('Tumblr','bw-kidxtore'),
		    'social' => 'tumblr',
		    'number' => '1',
			),
		array(
			'title'  => esc_html__('Email','bw-kidxtore'),
		    'social' => 'envelope',
		    'number' => '1',
			),
		);
...
?>
<?php if(!empty($style) && $style == 'popup') echo '<a href="#" class="popup-share"><i class="icon-bzo icon-bzo-share"><i class="path1"></i></i><span>'.esc_html__('Share','bw-kidxtore').'</span></a>';?>
		...
		foreach ($list as $value) {
			...
			switch ($value['social']) { 
				...
				case 'envelope':
					...
					$html_list .='<li><a target="_blank" data-social="'.esc_attr($value['social']).'" title="'.esc_attr($value['title']).'" href="mailto:?subject='.esc_attr__("I wanted you to see this site&amp;body=Check out this site",'bw-kidxtore').' '.get_the_permalink().'">
