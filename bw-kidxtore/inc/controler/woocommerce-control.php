...
if(!function_exists('bzotech_wishlist_compare_link_single_product')){
     function bzotech_wishlist_compare_link_single_product(){
        ...
        if(class_exists('YITH_WCWL') || class_exists('YITH_Woocompare') || ((isset($check_share[$post_type]) && $check_share[$post_type] == '1'))){
...
if(!function_exists('bzotech_woocommerce_thumbnail_loop')){
    ...
    function bzotech_woocommerce_thumbnail_loop($size, $animation = '', $echo = true) {
        ...
        if (has_post_thumbnail()) {
            ...
            $img_url = wp_get_attachment_image_url($thumbnail_id, $size);
            $img_url_srcset = wp_get_attachment_image_srcset($thumbnail_id, $size);

            // Validate src and srcset — fallback to full image if broken or missing
            ...
            if (is_wp_error($response) || wp_remote_retrieve_response_code($response) == 404) {
                ...
                $img_url_srcset = wp_get_attachment_image_srcset($thumbnail_id, 'full');

                // Rebuild image HTML
                $img_html = '<img src="' . esc_url($img_url) . '" alt="' . esc_attr(get_the_title()) . '" loading="lazy" decoding="async" class="wp-post-image" srcset="' . esc_attr($img_url_srcset) . '">';
            } else {
                // Use WordPress image generator (corrects srcset)
                $img_html = wp_get_attachment_image($thumbnail_id, $size, false, array(
                    'class' => 'wp-post-image',
                    'loading' => 'lazy',
                    'decoding' => 'async',
                ));
        ...
        if ($animation == 'rotate-thumb' || $animation == 'zoomout-thumb' || $animation == 'translate-thumb' || $animation == 'slider-thumb') {
            ...
            if ($animation == 'slider-thumb' && !empty($attachment_ids)) {
                ...
                $img_hover_html .= '<span class="img-thumbnail active" data-url="' . esc_url($img_url) . '" data-srcset="' . esc_attr($img_url_srcset) . '"></span>';
                ...
                foreach ($attachment_ids as $key => $value) {
                    ...
                    if (is_wp_error($response) || wp_remote_retrieve_response_code($response) == 404) {
                        $img_url_gallery = wp_get_attachment_image_url($value, 'full');
                        $srcset = wp_get_attachment_image_srcset($value, 'full');
                    }

                    $img_hover_html .= '<span class="img-gallery" data-url="' . esc_url($img_url_gallery) . '" data-srcset="' . esc_attr($srcset) . '"></span>';
                    if ($key == 4) break;
                }
                $img_hover_html .= '</span>';
            } else {
...
if(!function_exists('bzotech_product_label')){
    function bzotech_product_label($echo = true) {
        ...
        if($uppsell < 0) $html .=   '<span class="new">'.esc_html__("new",'bw-kidxtore').'</span>';
        if($trending_product=='1') $html .=   '<span class="trending">'.esc_html__("Trending",'bw-kidxtore').'</span>';
...
if(!function_exists('bzotech_get_bonus_html')){
    function bzotech_get_bonus_html($echo = true){
        ...
        <div class="bonus-info"><?php echo sprintf(__('Bonus: %s%s', 'bw-kidxtore'), get_woocommerce_currency_symbol(),($regular_price - $sale_price)); ?></div>
    <?php }
}
    ...
    function bzotech_get_rating_html($echo = true, $count = true, $style = '',$star=''){
        ...
        if(!empty($count)){
            ...
            if($style == 'style2'){
                ...
                if($review_count==0) $review_count = esc_html__('No review','bw-kidxtore');
                else{
                    if($review_count == 1) {
...
if ( ! function_exists( 'bzotech_addtocart_link' ) ) {
    function bzotech_addtocart_link($data=[],$echo = true){
        ...
        if ( $product ) {                
            switch ($style) {
                case 'cart-icon':
                    if(!$icon) $icon = '<i class="las la-shopping-bag"></i>';
                    $text = '';
...
if(!function_exists('bzotech_load_shop')){
    function bzotech_load_shop() {
        ...
        }else echo '<div class="list-col-item">'.bzotech_get_template_post( 'content' , 'none' ).'</div>';
        echo    '</div>';
        if($shop_style == 'load-more' && $max_page > 1){
            $data_load = array(
                "args"        => $args,
    ...
    function bzotech_load_product_filter() {
        ...
        if(isset($pagination) && !empty($pagination)){?>
            <div class="products bzotech-row list-product-wrap js-content-main">
                ...
                else echo '<div class="filter-noresult-wrap"><div class="filter-noresult title18 text-center">'.esc_html__("No result found with current filter value.",'bw-kidxtore').'</div></div>';
                ?>
            </div>
            <?php
...
if (!empty($stock)) {
            ...
            <div class="flashsale">
                ...
                <div class="sold-info"><?php echo sprintf(__('Sold: <span>%s/%s</span>', 'bw-kidxtore'), $buy,($stock + $buy)); ?></div>
