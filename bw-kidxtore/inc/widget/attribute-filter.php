...
if(!class_exists('Bzotech_Attribute_Filter') && class_exists("woocommerce")){
    class Bzotech_Attribute_Filter extends WP_Widget {
        ...
        function __construct() {
            ...
            parent::__construct( false, esc_html__('BZOTECH Attribute Filter','bw-kidxtore'),
                ...
                if(!empty($terms)){
                    ...
                    switch ($attr->attribute_type){
                                    ...
                                    foreach ($terms as $term){
                                        ...
                                        if(is_object($term)){
                                            ...
                                            if(!empty($color)){
                                                ...
                                                echo sprintf(
                                                    '<a data-attribute="%s" data-term="%s" class="load-shop-ajax swatch swatch-color '.$class_white_color.' swatch-%s %s"  title="%s" href="%s" %s><span class="span-trong" '.bzotech_add_html_attr('background-color:'.$color).'></span></a>',
                                                    esc_attr($attribute),
                                                    esc_attr( $term->slug ),
                                                    esc_attr( $term->slug ),
                                                    $selected,
                                ...
                                if(is_array($terms)){
                                    foreach ($terms as $term){
                                        if(is_object($term)){
                                            ...
                                            if(!empty($label)){
                                                echo sprintf(
                                                    '<a data-attribute="%s" data-term="%s" class="load-shop-ajax swatch swatch-label swatch-%s %s" title="%s" data-value="%s" href="%s" %s><span class="label-attr">%s</span><span class="count">('.$term->count.')</span></a>',
                                                    esc_attr($attribute),
                                                    esc_attr( $term->slug ),
                                                    esc_attr( $term->slug ),
                                                    $selected,
                        ...
                        default :
                            ...
                            echo '<ul class="list-filter attribute-type-default filter_' . esc_attr($attribute) . '">';
                            if (is_array($terms)) {
                                foreach ($terms as $term) {
                                    if (is_object($term)) {

                                        // Check if this term is active
                                        $slug_value = strtolower(str_replace(' ', '-', $term->name));
                                        $active = in_array($slug_value, $current_values) ? 'active' : '';

                                        // Map term name to friendly display name
                                        switch ($term->name) {
                                            case "0TO18MONTHS": $term_name = "0 - 18 Months"; break;
                                            case "TWOTOFIVE": $term_name = "2 - 5 Years"; break;
                                            case "SIXTOEIGHT": $term_name = "6 - 8 Years"; break;
                                            case "NINETOELEVEN": $term_name = "9 - 11 Years"; break;
                                            case "TWELVETOSEVENTEEN": $term_name = "12 - 17 Years"; break;
                                            case "OVER18": $term_name = "18+ Years"; break;
                                            default: $term_name = $term->name; break;
                                        }
