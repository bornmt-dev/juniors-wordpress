...
<?php echo '<div '.$item_inner.'>';?>
        <?php if($item_thumbnail == 'yes' && has_post_thumbnail()):?>
            <div class="post-thumb">
                ...
                <div class="date-thumb"><?php echo '<span class="post-date">'.get_the_date('d').'</span><span class="post-month"> '.get_the_date('M').'</span>'; ?></div>
            </div>
        <?php endif?>
        <div class="post-info">
            ...
            <?php if($item_title == 'yes'):?><h3 class="title16 post-title font-semibold"><a href="<?php echo esc_url(get_the_permalink()) ?>"><?php the_title()?></a></h3><?php endif ?>
            
            <?php if($item_excerpt == 'yes') echo '<p class="desc color-title">'.bzotech_substr(get_the_excerpt(),0,$excerpt).'</p>';?>
