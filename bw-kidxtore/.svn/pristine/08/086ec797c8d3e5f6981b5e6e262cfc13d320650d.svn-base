...
if(!class_exists('Bzotech_BlogListPostsWidget'))
{
    class Bzotech_BlogListPostsWidget extends WP_Widget {
        ...
        function __construct() {
            ...
            parent::__construct( false, esc_html__('BZOTECH Posts List','bw-kidxtore'),
                array( 'description' => esc_html__( 'Posts list', 'bw-kidxtore' ), ));

            $this->default=array(
                'title'=>esc_html__('List Posts','bw-kidxtore'),
                'posts_per_page'=>8,
                'style'=>'default',
                'category'=>'',
                'order'=>'desc',
        ...
        function form( $instance ) {
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id( 'posts_per_page' )); ?>"><?php esc_html_e( 'Post Number' ,'bw-kidxtore'); ?></label>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id( 'order_by' )); ?>"><?php esc_html_e( 'Order By' ,'bw-kidxtore'); ?></label>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id( 'order' )); ?>"><?php esc_html_e( 'Order' ,'bw-kidxtore'); ?></label>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id( 'category' )); ?>"><?php esc_html_e( 'Category' ,'bw-kidxtore'); ?></label>

                <?php wp_dropdown_categories(array(
                    'selected'=>$category,
                    'show_option_all'=>esc_html__('--- Select ---','bw-kidxtore'),
                    'name'  =>$this->get_field_name( 'category' )
                )); ?>
            </p>

            <p>
                <label for="<?php echo esc_attr($this->get_field_id( 'number_row' )); ?>"><?php esc_html_e( 'Number product in item silder (Default: 4)' ,'bw-kidxtore'); ?></label>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id( 'image_size' )); ?>"><?php esc_html_e( 'Custom image size (Example: enter size in pixels : 200x100 (Width x Height))' ,'bw-kidxtore'); ?></label>
