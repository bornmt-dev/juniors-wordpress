        ...
        <?php echo '<div '.$wdata->get_render_attribute_string( 'elbzotech-inner' ).'>';?>
            ...
            foreach (  $list_testimonial as $key => $item ) {
                ...
                echo '<div '.$wdata->get_render_attribute_string( 'elbzotech-item-slider-'.$style.'-'.$key ).'><div class="item-slider-global-'.$style.'">';
                    echo  '<div class="content-slider-custom client-say">'.bzotech_parse_text_editor( $item['content']).'</div>';
                    echo '<div class="flex-wrapper client-infos">';
                        ...
                        echo '<div class="client-info">';
                            if(!empty($item['title'])) echo '<h3 class="title18 font-semibold">'.$item['title'].'</h3>';
