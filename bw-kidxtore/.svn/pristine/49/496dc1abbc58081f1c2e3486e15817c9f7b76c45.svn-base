            ...
            foreach (  $list_categories5 as $key => $item ) {
                ...
                echo '<div '.$wdata->get_render_attribute_string( 'elbzotech-item-slider-'.$style.'-'.$key ).'><div class="item-slider-global-'.$style.'">';
                    ...
                    if(!empty($item['template'])) 
                       ...
                       echo '<a class="adv-thumb-link-cate adv-thumb-link" '.$link_html.'><div class="cate-img">'.$image_html.'</div></a>
        <div class="info"><h2 class="info-title"><a '.$link_html.'>'.$title_html.'</a></h2>'.$count_html.'</div>';
                    }  
