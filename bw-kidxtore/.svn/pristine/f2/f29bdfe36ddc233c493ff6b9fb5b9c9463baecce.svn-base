...
<?php echo '<div '.$wdata->get_render_attribute_string( 'elbzotech-wrapper' ).'>';?>
    ...
    <?php echo '<div '.$wdata->get_render_attribute_string( 'elbzotech-wrapper-slider' ).'>';?>
        <?php echo '<div '.$wdata->get_render_attribute_string( 'elbzotech-inner' ).'>';?>
            <?php 
            $i = 0;
            foreach (  $list_categories7 as $key => $item ) {
                $i++;
                ...
                echo '<div '.$wdata->get_render_attribute_string( 'elbzotech-item-slider-'.$style.'-'.$key ).'><div class="item-slider-global-'.$style.' item-'.$i.'">';            
                    ...
                    if(!empty($item['content'])){
                     ...
                     if(!empty($item['image']['url'])){
                        $image_html = '<span class="image-cate adv-thumb-link">'.wp_get_attachment_image( $item['image']['id'] ,'full').'</span>';
                        if(!empty($item['image_hover']['url'])) $image_html .= '<span class="image-hover">'.wp_get_attachment_image( $item['image_hover']['id'] ,'full').'</span>';
                    }
                    ...
                    if(!empty($item['template'])) 
                       ...
                       echo '<div class="cate-img zoom-image overflow-hidden"><a class="adv-thumb-link-cate" '.$link_html.'>'.$image_html.'</a></div>
        <div class="info"><h2 class="info-title color-title font-medium"><a '.$link_html.'>'.$title_html.'</a></h2><div class="desc">'.$desc_html.'</div></div>';
                    }  
                ...
                $wdata->remove_render_attribute( 'data_link', 'href', $item['link']['url']);
