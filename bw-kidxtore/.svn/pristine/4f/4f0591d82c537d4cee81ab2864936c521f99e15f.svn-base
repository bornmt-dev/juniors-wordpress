...
if($check):
	...
	<div <?php echo ''.$wdata->get_render_attribute_string('wrapper');?> >
		<div class="elbzotech-mailchimp-form content-popup-mailchimp flex-wrapper align_items-center">
			...
			<div class="adv-thumb-link"><?php echo Group_Control_Image_Size::get_attachment_image_html( $settings);?></div>
			<div class="info-mailchimp bzotech-scrollbar">
				<?php if(!empty($title)) echo '<h3 class="title title34 font-title color-title font-semibold">'.$title.'</h3>'; ?>
				...
				<div class="text-center dont-show"><input type="checkbox" id="close-newsletter"> <label for="close-newsletter"><?php esc_html_e("Don’t show this pop-up again",'bw-kidxtore')?></label></div>
			</div>
		</div>
	</div>
<?php endif;